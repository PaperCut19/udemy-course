...
app.put("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id); //CRIS/ we're getting the id of the joke that the user requested to have access to
  const replacementJoke = { //CRIS/ we're creating a brand new joke object with the user's data
    id: id,
    jokeText: req.body.text,
    jokeType: req.body.type
  };

  const searchIndex = jokes.findIndex((joke) => joke.id === id); //CRIS/ we're finding which joke object in the array has the id the user wanted
  jokes[searchIndex] = replacementJoke; //CRIS/ we're replacing the joke inside the array with the one the user sent
  res.json(replacementJoke); //CRIS/ we're sending the user the joke they sent us in JSON stringified format
})
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) { //CRIS/ if the joke exists, then use .splice to remove the joke that the user wanted to remove
    jokes.splice(searchIndex, 1);
    res.sendStatus(200); //CRIS/ this will tell the client/user that everything happened successfully
  } else {
    res
      .status(404)
...
app.delete("/all", (req, res) => {
  const userKey = req.query.key; //CRIS/ get access to the api key that the user sent using a query parameter
  if (userKey === masterKey) { //CRIS/ compare the user's API key to the master key, if it matches, we make the jokes array empty
    jokes = [];
    res.sendStatus(200);
  } else {
    res
